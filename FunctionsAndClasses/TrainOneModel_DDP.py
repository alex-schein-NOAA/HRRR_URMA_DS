...
def TrainOneModel_DDP(current_model_attrs,
    ...
    for epoch in range(1,upper_bound):
        ...
        ### ADDED FROM RON'S CODE (2025/11/17)
        # Gather epoch loss from all ranks for scheduler
        ...
        if rank==0:
            ...
            print(f"End of epoch {epoch} | Average loss for epoch = {avg_epoch_loss:.5f} | Time for epoch = {time.time()-start:.1f} sec | LR = {scheduler.get_last_lr()}")
